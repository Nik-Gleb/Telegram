...
public class OverlaysView extends View implements ProfileGalleryView.Callback {
    ...
    public OverlaysView(ProfileActivity mProfileActivity, Context context) {
        super(context);
        this.mProfileActivity = mProfileActivity;
        statusBarHeight = mProfileActivity.getActionBar().getOccupyStatusBar() && !mProfileActivity.isInBubbleMode() ? AndroidUtilities.statusBarHeight : 0;
        setVisibility(GONE);
    ...
    protected void onDraw(Canvas canvas) {
        ...
        selectedPosition = mProfileActivity.avatarsViewPager.getRealPosition();
        ...
        if (count > 1 && count <= 20) {
            if (mProfileActivity.overlayCountVisible == 0) {
                alpha = 0.0f;
                mProfileActivity.overlayCountVisible = 3;
            } else if (mProfileActivity.overlayCountVisible == 1) {
                alpha = 0.0f;
                mProfileActivity.overlayCountVisible = 2;
            }
            if (mProfileActivity.overlayCountVisible == 2) {
            ...
            int y = AndroidUtilities.dp(4) + (Build.VERSION.SDK_INT >= 21 && !mProfileActivity.isInBubbleMode() ? AndroidUtilities.statusBarHeight : 0);
            for (int a = 0; a < count; a++) {
                ...
                if (a == previousSelectedPotision && Math.abs(previousSelectedProgress - 1.0f) > 0.0001f) {
                    ...
                    if (mProfileActivity.avatarsViewPager.isCurrentItemVideo()) {
                        progress = currentProgress = mProfileActivity.avatarsViewPager.getCurrentItemProgress();
                        if (progress <= 0 && mProfileActivity.avatarsViewPager.isLoadingCurrentVideo() || currentLoadingAnimationProgress > 0.0f) {
                ...
                if (a != selectedPosition) {
                    if (mProfileActivity.overlayCountVisible == 3) {
            ...
            } else if (mProfileActivity.overlayCountVisible == 3) {
